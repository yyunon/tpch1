...
int tpch(int argc, char **argv, int k_t)
{
  ...
  for (int i = 0; i < nKernels; i++)
  {
    ...
    if (k_t == 0)
      fletcher::ReadRecordBatchesFromFile("/mnt/9a3ce8e2-5558-402f-b9b0-4253be72229d/THESIS_DATASETS/2021_05_19_18_53/l_recordbatch.rb", &batches); // 100 rows
    else if (k_t == 1)
      fletcher::ReadRecordBatchesFromFile("/mnt/9a3ce8e2-5558-402f-b9b0-4253be72229d/THESIS_DATASETS/2021_05_19_18_53/l_recordbatch.rb", &batches); //1000rows
  }
  ...
  status = kernel.GetReturn(&return_value_1, &return_value_0);
  std::cout << "Number of rows: " << return_value_1 <<"\n";
  // Take the data back.
  const int device_buffer_offset = 9;
  if (k_t == 1) // only copy for latest batch
  {
    ...
    platform->CopyDeviceToHost(context->device_buffer(device_buffer_offset + 4).device_address, res_sum_qty_data, sizeof(double) * num_rows);
    platform->CopyDeviceToHost(context->device_buffer(device_buffer_offset + 5).device_address, res_sum_base_price_data, sizeof(double) * num_rows);
    platform->CopyDeviceToHost(context->device_buffer(device_buffer_offset + 6).device_address, res_sum_disc_data, sizeof(double) * num_rows);
    platform->CopyDeviceToHost(context->device_buffer(device_buffer_offset + 7).device_address, res_sum_charge_data, sizeof(double) * num_rows);
    platform->CopyDeviceToHost(context->device_buffer(device_buffer_offset + 8).device_address, res_avg_qty_data, sizeof(double) * num_rows);
    platform->CopyDeviceToHost(context->device_buffer(device_buffer_offset + 9).device_address, res_avg_price_data, sizeof(double) * num_rows);
    platform->CopyDeviceToHost(context->device_buffer(device_buffer_offset + 10).device_address, res_avg_disc_data, sizeof(double) * num_rows);
